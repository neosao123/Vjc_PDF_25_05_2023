...
exports.totalCustomerOrders = async (req, res) => {
    try {
        let JobCards = await basicInfoModel.find({ basicInfoStep: true, detailInfoStep: true, customweRequestStep: true, jobDetailsStep: true, otherDetailsStep: true });
        let orderNo = [];
        JobCards.map((data) => {
        ...
        if (JobCards.length < 0) {
            ...
            res.status(200).json({ err: 200, msg: "Data found", totalCustomerOrders: JobCards.length, data: finalResult });
        }
    } catch (error) {
        res.status(500).json({ err: 500, msg: error.toString() })
    }
